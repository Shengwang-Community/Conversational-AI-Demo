...
5. 是否更新了 `PROJECT_STATE.md` 中需要同步的状态，并在 workflow 场景回显了 `[STATE]`
...
- [ ] 任务阶段、角色路由、`PLAN_FROZEN` / `CURRENT_ROLE` / `WORKFLOW_STATUS` 语义一致
...
- [ ] `single` / `single + reviewer` 路由已明确要求最小 Contract 与 reviewer 交接
- [ ] 纯 `single` 路由有明确完成态写回规则，不会停留在 `executor`
...
- [ ] `WORKFLOW_STATUS` 与真实阶段一致，已完成任务未被保留为可自动 continue 态
