...
class PhotoView @JvmOverloads constructor(
    ...
    private fun animateScale(targetScale: Float, shouldCenter: Boolean = true) {
        scaleAnimator?.cancel()
        ...
        scaleAnimator = android.animation.ValueAnimator.ofFloat(currentScale, targetScale).apply {
            duration = SCALE_ANIMATION_DURATION
            addUpdateListener { animator ->
                val scale = animator.animatedValue as Float
                val scaleRatio = scale / currentScale
                
                val newTransX = currentTransX * scaleRatio + viewCenterX * (1 - scaleRatio)
                val newTransY = currentTransY * scaleRatio + viewCenterY * (1 - scaleRatio)
    ...
    private fun animateScaleWithBoundary(targetScale: Float, shouldCenter: Boolean = true) {
        scaleAnimator?.cancel()
        ...
        val currentBounds = calculateImageBounds(originalBitmap)
        ...
        val finalTransform = calculateFinalTransform(
            originalBitmap, currentCenterX, currentCenterY, targetScale
        )
        ...
        scaleAnimator = android.animation.ValueAnimator.ofFloat(0f, 1f).apply {
            ...
            addUpdateListener { animator ->
                ...
                val scaleRatio = currentScale / startScale
                
                matrix.set(startMatrix)
                matrix.postScale(scaleRatio, scaleRatio, currentCenterX, currentCenterY)
                
                if (finalTransform.needsAdjustment) {
                    val adjustX = finalTransform.adjustX * progress
                    val adjustY = finalTransform.adjustY * progress
                    matrix.postTranslate(adjustX, adjustY)
                }
                
                this@PhotoView.currentScale = currentScale
                imageMatrix = matrix
            }
            start()
        }
    ...
    private fun calculateFinalTransform(
        bitmap: Bitmap,
        centerX: Float,
        centerY: Float,
        targetScale: Float
        ...
        testMatrix.postScale(scaleRatio, scaleRatio, centerX, centerY)
        
        val testBounds = calculateImageBounds(bitmap)
        testMatrix.mapRect(testBounds)
        ...
        when {
            testBounds.width() <= viewWidth -> {
                adjustX = (viewWidth / 2f) - testBounds.centerX()
                needsAdjustment = true
            }
            testBounds.left > 0 -> {
                adjustX = -testBounds.left
                needsAdjustment = true
            }
            testBounds.right < viewWidth -> {
                adjustX = viewWidth - testBounds.right
                needsAdjustment = true
            }
        }
        
        when {
            testBounds.height() <= viewHeight -> {
                adjustY = (viewHeight / 2f) - testBounds.centerY()
                needsAdjustment = true
            }
            testBounds.top > 0 -> {
                adjustY = -testBounds.top
                needsAdjustment = true
            }
            testBounds.bottom < viewHeight -> {
                adjustY = viewHeight - testBounds.bottom
                needsAdjustment = true
            }
        }
        
        return FinalTransform(needsAdjustment, adjustX, adjustY)
    }
    
    private data class FinalTransform(
