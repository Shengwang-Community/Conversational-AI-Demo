...
class SSOWebViewActivity : BaseActivity<CovActivitySsoBinding>() {

    override fun getViewBinding(): CovActivitySsoBinding {
        return CovActivitySsoBinding.inflate(layoutInflater)
    }
    ...
    inner class WebAppInterface(private val context: Context) {
        ...
        fun handleResponse(response: String) {
            ...
            if (!response.startsWith("Error")) {
                ...
                runOnUiThread {
                    setResult(RESULT_OK, Intent().putExtra("token", response))
                    finish()
                }
            } else {
