...
class CovLivingBottomView @JvmOverloads constructor(
    ...
    fun updateMicrophoneView(isLocalAudioMuted: Boolean, isLightBackground: Boolean = false) {
        if (isLocalAudioMuted) {
            binding.btnMic.setImageResource(R.drawable.scene_detail_microphone0)
            binding.btnMic.setBackgroundResource(
                R.drawable.btn_bg_brand_white_selector
            )
        } else {
            binding.btnMic.setImageResource(R.drawable.agent_user_speaker)
            if (isLightBackground) {
                binding.btnMic.setBackgroundResource(R.drawable.btn_bg_brand_black4_selector)
            } else {
                binding.btnMic.setBackgroundResource(R.drawable.btn_bg_block1_selector)
            }
        }
    }
    ...
    fun updateCameraView(isPublish: Boolean) {
        if (isPublish) {
            binding.btnCamera.setImageResource(R.drawable.scene_detail_camera_on)
        } else {
            binding.btnCamera.setImageResource(R.drawable.scene_detail_camera_off)
        }
    }
    ...
    fun updateImageButtonWithAnimation(isPublishVideo: Boolean) {
        ...
        if (isPublishVideo) {
            ...
            if (ivAddPic.isVisible) {
                ...
                val outAnim =
                    AnimationUtils.loadAnimation(context, io.agora.scene.convoai.R.anim.slide_up_out)
                ...
                val inAnim =
                    AnimationUtils.loadAnimation(context, io.agora.scene.convoai.R.anim.slide_up_in)
    ...
    fun updateButtonBackgrounds(isLight: Boolean) {
        if (isLight) {
            binding.btnEndCall.setBackgroundResource(R.drawable.btn_bg_brand_black4_selector)
            binding.btnCamera.setBackgroundResource(R.drawable.btn_bg_brand_black4_selector)
            binding.btnImageContainer.setBackgroundResource(R.drawable.btn_bg_brand_black4_selector)
        } else {
            binding.btnEndCall.setBackgroundResource(R.drawable.btn_bg_block1_selector)
            binding.btnCamera.setBackgroundResource(R.drawable.btn_bg_block1_selector)
            binding.btnImageContainer.setBackgroundResource(R.drawable.btn_bg_block1_selector)
        }
    }
     ...
     */
    fun updateCallingControlsVisibility(connectionState: AgentConnectionState) {
        when (connectionState) {
            AgentConnectionState.IDLE -> {
                binding.llCalling.visibility = INVISIBLE
                binding.btnJoinCall.visibility = VISIBLE
            }
            AgentConnectionState.CONNECTING -> {
                binding.llCalling.visibility = VISIBLE
                binding.btnJoinCall.visibility = INVISIBLE
            }
